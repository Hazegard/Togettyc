...
func (d *Decoder) DecodeFrame() (*ttyrec.Frame, error) {
	return d.decodeFrame(false)
}

func (d *Decoder) decodeFrame(discard bool) (*ttyrec.Frame, error) {
	var (
		f   ttyrec.Frame
		n   int64
		err error
...
func (d *Decoder) DecodeStream() (<-chan *ttyrec.Frame, StopFunc) {
	var (
		output = make(chan *ttyrec.Frame)
		stop   = make(chan struct{})
	)

	go func(output chan<- *ttyrec.Frame, stop chan struct{}) {
		defer close(output)
